...
// Remove the `iconName` field from every stored Goal.
//
// A goal's icon must always come purely from its category — a goal can never
// hold a custom icon. This migration drops the `iconName` field from the Goal
// storage record so no custom icon can be stored behind the scenes. Existing
// goals lose any previously stored icon value (intentional data loss — the
// field is being removed entirely).
//
// OldActor matches the NewActor of 20260915_010000.mo (the previous
// migration). NewActor drops `iconName` from Goal. Types are inlined — no
// project imports.
...
module {
  ...
  // Old Goal — carries the iconName field.
  type OldGoal = {
    id : Nat;
    owner : Principal;
    var goalId : ?Nat;
  ...
  public func migration(old : OldActor) : NewActor {
    // Rebuild goals, dropping the iconName field from each record.
