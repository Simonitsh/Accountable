...
mixin (
  ...
  public shared ({ caller }) func createGoal(request : GoalTypes.CreateGoalRequest) : async { #ok : GoalTypes.GoalPublic; #err : Text } {
    if (caller.isAnonymous()) Runtime.trap("Anonymous callers cannot create goals");
    let goalStore = Goals.GoalStore(goals, obstacleTemplates, nextGoalId, nextObstacleTemplateId);
